...
// A fake Gmail labels API seeded with the two parents (proper-cased) + records creates.
...
test("resolve without create: children that don't exist come back empty", async () => {
  ...
  const r = await resolveRonnieLabels({ gmail, interestingId: "L_INT", bulkId: "L_BULK", create: false });
...
test("resolve with create: builds nested names under the real parent names", async () => {
  const gmail = fakeGmail();
  const r = await resolveRonnieLabels({ gmail, interestingId: "L_INT", bulkId: "L_BULK", create: true });
  // Names are parentName/Title, so they nest in Gmail's sidebar.
  assert.deepEqual(
    gmail._created.sort(),
    ["Bulk/Banking", "Bulk/Entropy", "Bulk/Jobs", "Interesting/Banking", "Interesting/Entropy", "Interesting/Jobs", "Interesting/Taxes"].sort()
  );
